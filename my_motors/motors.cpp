...
motors::~motors()
{
    std::cout << "motors destructor called" << std::endl;
}

int motors::set_led(uint8_t led, uint8_t red, uint8_t green, uint8_t blue) {
...
int motors::set_motor_power(uint8_t port, int8_t power) {
    spi_array_out[0] = Address;
...
int motors::set_grove_type(uint8_t port, uint8_t type) {
...
int motors::set_grove_mode(uint8_t pin, uint8_t mode) {
    spi_array_out[0] = Address;
...
int motors::set_grove_state(uint8_t pin, uint8_t state) {
    spi_array_out[0] = Address;
...
int motors::set_grove_pwm_duty(uint8_t pin, float duty) {
    if (duty > 100) {
        duty = 100;
    }
    else if (duty < 0) {
    ...
    uint16_t duty_value = (uint16_t)(duty * 10);
    spi_array_out[0] = Address;
    spi_array_out[1] = GPGSPI_MESSAGE_SET_GROVE_PWM_DUTY;
    spi_array_out[2] = pin;
    spi_array_out[3] = (uint8_t)((duty_value >> 8) & 0xFF);
    spi_array_out[4] = (uint8_t)(duty_value & 0xFF);
    return spi_transfer_array(5, spi_array_out, spi_array_in);
}

int motors::set_grove_pwm_frequency(uint8_t port, uint16_t freq) {
    spi_array_out[0] = Address;
    spi_array_out[1] = GPGSPI_MESSAGE_SET_GROVE_PWM_FREQUENCY;
    spi_array_out[2] = port;
    spi_array_out[3] = (uint8_t)((freq >> 8) & 0xFF);
    spi_array_out[4] = (uint8_t)(freq & 0xFF);
    return spi_transfer_array(5, spi_array_out, spi_array_in);
}

int motors::grove_i2c_transfer(uint8_t port, i2c_struct_t* i2c_struct) {
    double Timeout = get_time() + 0.005;
    bool Continue = false;
    int error;
    while (!Continue) {
        if ((error = (uint8_t)grove_i2c_start(port, i2c_struct))) {
            if (get_time() >= Timeout) {
                return error;
            }
        }
    ...
    usleep((int)(DelayTime * 1000000)); // delay for as long as it will take to do the I2C transaction.
    Timeout = get_time() + 0.005;  // timeout after 5ms of failed attempted reads

    while (true) {
        if ((error = (uint8_t)get_grove_value(port, i2c_struct))) {
            if (get_time() >= Timeout) {
                return error;
            }
        }
...
int motors::grove_i2c_start(uint8_t port, i2c_struct_t* i2c_struct) {
    uint8_t msg_type;
    uint8_t port_index;
    switch (port) {
    case GROVE_1:
    ...
    spi_array_out[2] = (uint8_t)((i2c_struct->address & 0x7F) << 1);
    ...
    if (int error = spi_transfer_array((uint8_t)(5 + i2c_struct->length_write), spi_array_out, spi_array_in)) {
        return error;
    }
...
int motors::get_grove_value(uint8_t port, void* value_ptr) {
    uint8_t msg_type;
    uint8_t port_index;
    switch (port) {
    case GROVE_1:
    ...
    case GROVE_TYPE_US:
        ...
        break;
    case GROVE_TYPE_I2C:
        spi_transfer_length = (uint8_t)(6 + GroveI2CInBytes[port_index]);
        break;
    default:
    ...
    if (GroveType[port_index] == GROVE_TYPE_IR_DI_REMOTE) {
        ...
        Value->mm = (uint16_t)(((spi_array_in[6] << 8) & 0xFF00) | (spi_array_in[7] & 0xFF));
        Value->cm = (float)(Value->mm / 10.0);
        Value->inch = (float)(Value->mm / 25.4);
    }
...
uint8_t motors::get_grove_state(uint8_t pin) {
    uint8_t value;
    get_grove_state(pin, value);
    return value;
}

int motors::get_grove_state(uint8_t pin, uint8_t& value) {
    value = 0;
    uint8_t msg_type;
    switch (pin) {
    case GROVE_1_1:
...
float motors::get_grove_voltage(uint8_t pin) {
    float value;
    get_grove_voltage(pin, value);
    return value;
}

int motors::get_grove_voltage(uint8_t pin, float& value) {
    value = 0;
    uint8_t msg_type;
    switch (pin) {
    case GROVE_1_1:
    ...
    if (spi_array_in[3] == 0xA5) {
        if (spi_array_in[4] == GROVE_STATUS_VALID_DATA) {
            value = (float)((((spi_array_in[5] << 8) & 0xFF) | (spi_array_in[5] & 0xFF)) / 1000.0);
            return ERROR_NONE;
        }
        else {
            return ERROR_GROVE_DATA_ERROR;
...
uint16_t motors::get_grove_analog(uint8_t pin) {
    uint16_t value;
    get_grove_analog(pin, value);
    return value;
}

int motors::get_grove_analog(uint8_t pin, uint16_t& value) {
    value = 0;
    uint8_t msg_type;
    switch (pin) {
    case GROVE_1_1:
...
int motors::set_motor_limits(uint8_t port, uint8_t power, uint16_t dps) {
    spi_array_out[0] = Address;
    spi_array_out[1] = GPGSPI_MESSAGE_SET_MOTOR_LIMITS;
    spi_array_out[2] = port;
    spi_array_out[3] = power;
    spi_array_out[4] = (uint8_t)((dps >> 8) & 0xFF);
    spi_array_out[5] = (uint8_t)(dps & 0xFF);
    return spi_transfer_array(6, spi_array_out, spi_array_in);
}

int motors::detect(bool critical) {
    char ErrorStr[100];
    char str[21];
    int error;
    // assign error to the value returned by get_manufacturer, and if not 0:
    if ((error = (uint8_t)get_manufacturer(str))) {
        if (critical) {
            fatal_error("detect error: get_manufacturer failed. Perhaps the motors is not connected, or the address is incorrect.");
        }
        else {
            return error;
        }
    ...
    if (strstr(str, "GoPiGo3") != str) {
        if (critical) {
            fatal_error("detect error: get_board string is not 'motors'");
        }
        else {
    ...
    if (strstr(str, FIRMWARE_VERSION_REQUIRED) != str) {
        if (critical) {
            sprintf(ErrorStr, "detect error: motors firmware needs to be version %sx but is currently version %s", FIRMWARE_VERSION_REQUIRED, str);
...
int motors::get_version_firmware(char* str) {
    uint32_t value;
    // assign error to the value returned by spi_read_32, and if not 0:
...
int motors::spi_read_8(uint8_t msg_type, uint8_t& value) {
    value = 0;
    spi_array_out[0] = Address;
    spi_array_out[1] = msg_type;
    // assign error to the value returned by spi_transfer_array, and if not 0:
...
int motors::spi_read_16(uint8_t msg_type, uint16_t& value) {
    value = 0;
    spi_array_out[0] = Address;
    spi_array_out[1] = msg_type;
    // assign error to the value returned by spi_transfer_array, and if not 0:
    ...
    value = (uint16_t)((spi_array_in[4] << 8) | spi_array_in[5]);
    return ERROR_NONE;
}

int motors::spi_read_32(uint8_t msg_type, uint32_t& value) {
    value = 0;
    spi_array_out[0] = Address;
    spi_array_out[1] = msg_type;
    // assign error to the value returned by spi_transfer_array, and if not 0:
...
int motors::spi_read_string(uint8_t msg_type, char* str, uint8_t chars) {
    ...
    spi_array_out[1] = msg_type;
    // assign error to the value returned by spi_transfer_array, and if not 0:
    if (int error = spi_transfer_array((uint8_t)(chars + 4), spi_array_out, spi_array_in)) {
        return error;
    }
    if (spi_array_in[3] != 0xA5) {
        return ERROR_SPI_RESPONSE;
...
int motors::spi_write_32(uint8_t msg_type, uint32_t value) {
    spi_array_out[0] = Address;
    spi_array_out[1] = msg_type;
    spi_array_out[2] = (uint8_t)((value >> 24) & 0xFF);
...
int motors::offset_motor_encoder(uint8_t port, int32_t position) {
    spi_array_out[0] = Address;
    spi_array_out[1] = GPGSPI_MESSAGE_OFFSET_MOTOR_ENCODER;
    spi_array_out[2] = port;
    spi_array_out[3] = (uint8_t)((position >> 24) & 0xFF);
    spi_array_out[4] = (uint8_t)((position >> 16) & 0xFF);
    spi_array_out[5] = (uint8_t)((position >> 8) & 0xFF);
    spi_array_out[6] = (uint8_t)(position & 0xFF);
    return spi_transfer_array(7, spi_array_out, spi_array_in);
}

int32_t motors::get_motor_encoder(uint8_t port) {
    int32_t value;
    get_motor_encoder(port, value);
    return value;
}

int motors::get_motor_encoder(uint8_t port, int32_t& value) {
    uint8_t msg_type;
    switch (port) {
    case MOTOR_LEFT:
...
int motors::set_motor_position(uint8_t port, int32_t position) {
    spi_array_out[0] = Address;
